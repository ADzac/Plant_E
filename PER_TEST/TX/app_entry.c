...
void MX_APPE_Process(void)
{
  ...
  if (wakeupSource & PWR_WAKEUP_LPAWUR)
  {
    ...
    if (LPAWUR_Payload[0] != run) {
    	totalPacket[run] = packetReceived;

		run = LPAWUR_Payload[0];
		packetReceived = 0;
	}
    packetReceived++;
//	printf("LPAWUR data received: [ ");
	printf("Run %d : packet number : %d", LPAWUR_Payload[0],packetReceived);
	printf("\n\r");
	for (int k = 0; k< 5; k++){
		printf(" %d ", totalPacket[k]);
	}
	printf("\n\r");
