...
int main(void)
{
	...
	while (1)
	{
		...
		if (wakeupSource & PWR_WAKEUP_RTC) TempANDHumidSensor();
		if (wakeupSource & PWR_WAKEUP_LPAWUR) 	GotoRx(vectcTxBuff);
...
void HAL_RTCEx_WakeUpTimerEventCallback(RTC_HandleTypeDef *hrtc)
{
    ...
    printf("======================== New cycle ======================== \n\r");
    // Must reconfigure for next wakeup
	HAL_PWREx_EnableInternalWakeUpLine(PWR_WAKEUP_RTC, PWR_WUP_RISIEDG);
}
...
void configRTCWakeupTimer(uint32_t timeout)
{
    // For timeout in milliseconds, convert to seconds
    while (HAL_RTCEx_SetWakeUpTimer_IT(&hrtc, ((timeout/1000)*2048), RTC_WAKEUPCLOCK_RTCCLK_DIV16) != HAL_OK)
    {
        printf("Failed \n\r");
    }
    printf("Done\n\r");
...
static void MX_LPAWUR_Init(void)
{
	...
	NVIC_SetPriority(LPAWUR_IRQn, 0);
	NVIC_EnableIRQ(LPAWUR_IRQn);
